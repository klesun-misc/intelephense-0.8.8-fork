...
class NameResolver {
    ...
    get classBaseName() {
        ...
        let base = s.associated.find((x) => {
            return x.kind === symbol_1.SymbolKind.Class;
        });
    ...
    resolveNotFullyQualified(notFqn, kind, resolveStatic) {
        ...
        return pos < 0 ?
            this._resolveUnqualified(notFqn, kind ? kind : symbol_1.SymbolKind.Class) :
    ...
    matchImportedSymbol(text, kind) {
        if (kind !== symbol_1.SymbolKind.Constant) {
        ...
        for (let n = 0, l = this.rules.length; n < l; ++n) {
            ...
            if (s.name && s.kind === kind &&
                ((kind === symbol_1.SymbolKind.Constant && text === s.name) ||
                    (kind !== symbol_1.SymbolKind.Constant && text === s.name.toLowerCase()))) {
                return s;
            }
        }
        return null;
    ...
    _resolveQualified(name, pos) {
        let s = this.matchImportedSymbol(name.slice(0, pos), symbol_1.SymbolKind.Class);
        return s ? s.associated[0].name + name.slice(pos) : this.resolveRelative(name);
    }
    _resolveUnqualified(name, kind) {
        if (kind === symbol_1.SymbolKind.Constructor) {
            kind = symbol_1.SymbolKind.Class;
        }
