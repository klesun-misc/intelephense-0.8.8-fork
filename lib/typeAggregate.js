...
var MemberMergeStrategy;
(function (MemberMergeStrategy) {
    MemberMergeStrategy[MemberMergeStrategy["None"] = 0] = "None";
    MemberMergeStrategy[MemberMergeStrategy["Override"] = 1] = "Override";
    MemberMergeStrategy[MemberMergeStrategy["Documented"] = 2] = "Documented";
    MemberMergeStrategy[MemberMergeStrategy["Base"] = 3] = "Base";
})(MemberMergeStrategy = exports.MemberMergeStrategy || (exports.MemberMergeStrategy = {}));
class TypeAggregate {
    ...
    isBaseClass(name) {
        ...
        let fn = (x) => {
            return x.kind === symbol_1.SymbolKind.Class && lcName === x.name.toLowerCase();
        };
    ...
    _resolveThisAndStaticReturnType(members, name) {
        ...
        for (let n = 0; n < members.length; ++n) {
            s = members[n];
            if ((s.kind & (symbol_1.SymbolKind.Method | symbol_1.SymbolKind.Property)) > 0 && s.doc && s.doc.type) {
    ...
    _classMembers(associated, strategy, predicate) {
        ...
        let p = predicate;
        let noPrivate = (x) => {
            return !(x.modifiers & symbol_1.SymbolModifier.Private) && (!predicate || predicate(x));
        };
        for (let n = 0; n < associated.length; ++n) {
            s = associated[n];
            if (s.kind === symbol_1.SymbolKind.Trait) {
                traits.push(s);
            }
            else if (s.children) {
    ...
    _mergeMembers(symbols, strategy) {
        ...
        for (let n = 0; n < symbols.length; ++n) {
            ...
            mapped = map[s.name];
            if (!mapped ||
                ((mapped.modifiers & symbol_1.SymbolModifier.Magic) > 0 && !(s.modifiers & symbol_1.SymbolModifier.Magic)) ||
    ...
    *_associatedIterator() {
        ...
        while ((stub = queue.shift())) {
            if (this._excludeTraits && stub.kind === symbol_1.SymbolKind.Trait) {
                continue;
            }
